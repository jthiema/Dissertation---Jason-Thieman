...
\begin{cabstract}
Appendix~\ref{Trigger_Efficiency_Scale_Factors} details the method and results of measuring trigger efficiency scale factors for the selection of top quark pair (\ttbar) events that decay via the dileptonic mode with dilepton (\ee, \emu, \mumu) final states.
The results, which were used by virtually all CMS Run II measurements analyzing \ttbar events that decay via the dileptonic mode, are crucial for correcting the trigger efficiencies in Monte Carlo simulations to match those observed in real data.
It discusses the datasets and triggers used, the criteria for object and event selection, and also covers statistical and systematic uncertainties associated with these measurements. 
Additionally, this appendix includes dedicated trigger efficiency scale factor measurements that were used specifically for investigating Lorentz Invariance Violation in \ttbar production.
\end{cabstract}
...
In order to accommodate a CMS search for a violation of Lorentz invariance (TOP-22-007), the dilepton trigger efficiency scale factors were measured one-dimensionally as a function of sidereal time using the same method outlined above, with the exception of using custom time-dependent corrections and not including the number of vertices partition as a source of event topology systematic uncertainty~\cite{CMS-PAS-TOP-22-007}.
